...
function updateGameLogic(dt)	
	...
	-- Deduct total costs
	playerMoney = playerMoney - totalPowerCost - InternetCosts
			...
			if contract.acceptButtonBounds then
				...
				if x >= btnBounds.x and x <= btnBounds.x + btnBounds.w and
				   y >= btnBounds.y and y <= btnBounds.y + btnBounds.h then
					...
					Contracts.addToWorkQueue(workQueue, contract, nodes, i)
...
function placeNode(variantId, x, y)
	...
	--If the node has an internet cost, add it to the global
	if variant.internetCostPerSecond then
		InternetCosts = InternetCosts + variant.internetCostPerSecond
	end
...
function DeleteNode(Node)
	-- Remove all connections to this node
	for i = #connections, 1, -1 do
		if connections[i].from == Node or connections[i].to == Node then
			table.remove(connections, i)
		end
	end
	...
	if Node.category == "rack" then
		for i = #connections, 1, -1 do
			if connections[i].isRackInternal and connections[i].rack == Node then
				table.remove(connections, i)
			end
		end
	end
	
	--If the node has an internet cost, subtract it from the global
	if Node.internetCostPerSecond then
		InternetCosts = InternetCosts - variant.internetCostPerSecond
	end
...
function createConnection(from, to, connType)
	...
	--Update power consumption when we make new connections instead of every update
